...
vaccine_49 <- vaccine_49 %>%
  mutate (no_change = ifelse (HEALTH.027 == 0, 1, 0), 
          ...
          effective_tv_news = ifelse (HEALTH.027 == 1 & HEALTH.029_1 == 1, 1, 0),
          safe_tv_news = ifelse (HEALTH.027 == 1 & HEALTH.029_2 == 1, 1, 0),
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -1, hjust = 0, size = 2.5, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for less likely to get vaccinated, n = 41")

vaccine_smy <- vaccine_49 %>%
  filter (more_likely_get == 1)%>%
  summarise (n = n (),
             effective_tv_news = sum (effective_tv_news, na.rm = T)/n, 
             safe_tv_news = sum (safe_tv_news, na.rm = T)/n, 
  ...
  pivot_longer (cols = c("effective_tv_news", "safe_tv_news", "safe_effective_friends_relatives", "do_things_only_vaccinated", "other_more_likely"), names_to = "condition", values_to = "prop")%>%
  mutate (count = prop * n, 
          moe = map2_dbl(prop, n, moe.p), 
          ci_low = prop-moe,
          ci_high = prop+moe)
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -1, hjust = 0, size = 2.5, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for more likely to get vaccinated, n = 187")
