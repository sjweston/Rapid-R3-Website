...
vaccine_parent <- vaccine %>%
   ...
   mutate (a.very_somewhat_likely_or_already_vaccinated = ifelse (HEALTH.022 %in% c(1, 2, 5), 1, 0),
           b.not_very_likely = ifelse (HEALTH.022 == 3, 1, 0),
           c.not_at_all_likely = ifelse (HEALTH.022 == 4, 1, 0))%>%
  group_by (Week)%>%
   summarise (n = n(),
              a.very_somewhat_likely_or_already_vaccinated = sum(a.very_somewhat_likely_or_already_vaccinated, na.rm = T)/n,
              b.not_very_likely = sum(b.not_very_likely, na.rm = T)/n, 
              c.not_at_all_likely = sum(c.not_at_all_likely, na.rm = T)/n,
              date = as.Date(min (StartDate)))%>%
   pivot_longer (cols = c("a.very_somewhat_likely_or_already_vaccinated", "b.not_very_likely", "c.not_at_all_likely"), names_to = "rsp", values_to = "prop")%>%
   mutate (count = prop * n, 
...
vaccine_child <- vaccine %>%
   ...
   mutate (a.very_somewhat_likely_or_already_vaccinated = ifelse (HEALTH.023 %in% c(1,2,5), 1, 0),
           b.not_very_likely = ifelse (HEALTH.023 == 3, 1, 0),
           c.not_at_all_likely = ifelse (HEALTH.023 == 4, 1, 0))%>%
  group_by (Week)%>%
   summarise (n = n(),
              a.very_somewhat_likely_or_already_vaccinated = sum(a.very_somewhat_likely_or_already_vaccinated, na.rm = T)/n,
              b.not_very_likely = sum(b.not_very_likely, na.rm = T)/n, 
...
vaccine_smy %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle(label = "Trend of changes - by race/ethnicity")+
...
vaccine_smy %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle(label = "Trend of changes - by poverty level")+
...
vaccine_parent %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle(label = "Trend of changes in PARENTS, by race/ethnicity & poverty")+
...
vaccine_child %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle(label = "Trend of changes in CHILDREN, by race/ethnicity & poverty")+
  facet_wrap(poverty_cat~ race_ethnic, nrow = 2)
```
...
vaccine_smy %>%
  ...
  geom_line() +
  labs (y = "Percentage")+
  ggtitle(label = "Trend of changes - by region")+
...
vaccine_smy %>%
  ...
  geom_errorbar(aes(xmin = ci_low*100,                        # Error bar 
                    ...
                    position = position_dodge(1)) +           # stacked bar chart: position = "stack"
  geom_text(aes(label = round(prop*100,2)),vjust = -0.5, hjust = 0, size = 2.5, color = "black",  position =  position_dodge(1))+
  facet_wrap (region~have_receive, nrow = 2)+
  theme(legend.position = "none")+
  ylab ("")
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -1, hjust = 0, size = 2.5, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for less likely to get vaccinated, n = 40")
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -1, hjust = 0, size = 2.5, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for more likely to get vaccinated, n = 186")
...
### 1.3.4 By race/ethnicity 

  * Note: We only included White families in the "reasons" analyses because of the limit of sample size of Black and Latinx families\
...
vaccine_smy <- vaccine_49 %>%
  filter (less_likely_get == 1 & race_ethnic == "White" & poverty_cat != "NA")%>%
  group_by (poverty_cat)%>%
  summarise (n = n (),
             unsafe_friends_relatives = sum (unsafe_friends_relatives, na.rm = T)/n, 
             unsafe_tv = sum (unsafe_tv, na.rm = T)/n, 
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -0.5, hjust = 0, size = 2, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for less likely to get vaccinated, White families only")+
  facet_wrap (~poverty_cat, nrow = 2)

vaccine_smy <- vaccine_49 %>%
  filter (more_likely_get == 1 & race_ethnic == "White" & poverty_cat != "NA")%>%
  group_by (poverty_cat)%>%
  summarise (n = n (),
             safe_effective_tv_news = sum (safe_effective_tv_news, na.rm = T)/n, 
             safe_effective_friends_relatives = sum (safe_effective_friends_relatives, na.rm = T)/n, 
...
vaccine_smy %>%
  ...
  geom_text(aes(label = round(prop*100,2)),vjust = -0.5, hjust = 0, size = 2, color = "black",  position =  position_dodge(1))+
  theme(legend.position = "none")+
  ggtitle(label = "Reasons for more likely to get vaccinated, White families only")+
  facet_wrap (~ poverty_cat, nrow = 2)
