...
```{r 0prelim-may29-4, eval = (which_demo == "race_cat")}
...
scored %>%
  select(contains("expect_"), all_of(demo_groups), income) %>%
  ...
  geom_text(aes(label = Count, x= Percent + 10), position = position_dodge(width = 1))+
...
```{r 0prelim-may29-6, out.width='100%'}
scored %>%
  select(contains("expect_"), all_of(demo_groups), median_income) %>%
...
scored %>%
  select(contains("expect_age"), all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 &!is.na(income)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo, -income) %>%
...
scored %>%
  select(contains("expect_age"), all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 &!is.na(income)) %>%
  select(-dvalue)%>%
...
scored %>%
  select(contains("expect_relation"), all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo, - income) %>%
...
scored %>%
  select(contains("expect_relation"), all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
...
```{r 0prelim-may29-14}
scored %>%
  select(same_cc, all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(same_cc) & !is.na(income) ) %>%
  select(-dvalue) %>%
  group_by(same_cc,income, demo) %>%
  summarize(Count = n()) %>%
...
```{r 0prelim-may29-15}
scored %>%
  select(same_cc, all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(same_cc) & !is.na(income) ) %>%
  select(-dvalue) %>%
...
```{r 0prelim-may29-17}
scored %>%
  select(willing_same_cc, all_of(demo_groups),income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(willing_same_cc)& !is.na(income)) %>%
  select(-dvalue) %>%
  group_by(willing_same_cc,income, demo) %>%
  summarize(Count = n()) %>%
  group_by(demo) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
...
```{r 0prelim-may29-18}
scored %>%
  select(willing_same_cc, all_of(demo_groups),median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(willing_same_cc)& !is.na(income)) %>%
  select(-dvalue) %>%
...
scored %>%
  select(contains("barrier_same_cc_"), all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo, -income) %>%
...
scored %>%
  select(contains("barrier_same_cc_"), all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
...
```{r 0prelim-may29-22}
scored %>%
  select(comfort_return, all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(comfort_return)& !is.na(income)) %>%
  select(-dvalue) %>%
  group_by(comfort_return, demo, income) %>%
  summarize(Count = n()) %>%
  group_by(demo, income) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
...
```{r 0prelim-may29-23}
scored %>%
  select(comfort_return, all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(comfort_return)& !is.na(income)) %>%
  select(-dvalue) %>%
...
scored %>%
  select(contains("know_cc"), all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo,-income) %>%
...
scored %>%
  select(contains("know_cc"), all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
...
```{r 0prelim-may29-28}
scored %>%
  select(lack_cc, all_of(demo_groups), income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(lack_cc) & !is.na(income)) %>%
  select(-dvalue) %>%
  group_by(lack_cc, income, demo) %>%
  summarize(Count = n()) %>%
  group_by(demo, income) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
...
```{r 0prelim-may29-29}
scored %>%
  select(lack_cc, all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(lack_cc) & !is.na(income)) %>%
  select(-dvalue) %>%
...
scored %>%
  select(contains("lostcc_", ignore.case = F), 
         all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
...
scored %>%
  select(contains("source_"), all_of(demo_groups), median_income) %>%
  rename(income = median_income) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(income)) %>%
  select(-dvalue)%>%
