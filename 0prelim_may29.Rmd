...
scored = scored %>%
  mutate(income = income/1000,
         ...
         #income = factor(poverty200, labels = c("high income","low income")),
         median_income = median_income/1000,
         median_income = cut(median_income, breaks = med_household_quantiles),
         poverty200 = factor(poverty200, labels = c("high income","low income")),
         poverty150 = factor(poverty150, labels = c("high income","low income")))
```
...
scored %>%
  select(contains("expect_"), poverty200) %>%
  select(-contains("age"), -contains("relation")) %>%
  filter(!is.na(poverty200)) %>%
  gather(type, value,  -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, type) %>%
...
```{r 0prelim-may29-9, out.width='100%'}
scored %>%
  select(contains("expect_"), poverty150) %>%
  select(-contains("age"), -contains("relation")) %>%
  filter(!is.na(poverty150)) %>%
  gather(type, value,  -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(poverty150, type) %>%
  summarize(Count = sum(value),
    Percent = 100*sum(value)/n()) %>%
  mutate(type = factor(type, 
                       levels = c("expect_center",
  ...
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = type, fill = poverty150))+
  geom_bar(stat = "identity", position = position_dodge(width = 1)) +
  geom_text(aes(label = round(Percent), x= Percent + 10), position = position_dodge(width = 1))+
  scale_fill_brewer(palette = "Dark2")+
  facet_grid(poverty150~.)+
  guides(fill = F) +
  labs(y = "")+
  theme_bw() +
  theme(legend.position = "top")
...
```{r 0prelim-may29-11, out.width='100%'}
scored %>%
...
scored %>%
  select(contains("expect_age"), poverty200) %>%
  filter(!is.na(poverty200)) %>%
  gather(type, value,  -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, type) %>%
...
```{r 0prelim-may29-14, out.width='100%'}
scored %>%
  select(contains("expect_age"), poverty150) %>%
  filter(!is.na(poverty150)) %>%
  gather(type, value,  -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(poverty150, type) %>%
...
```{r 0prelim-may29-16, out.width='100%'}
scored %>%
...
```{r 0prelim-may29-18, out.width='100%'}
scored %>%
  select(contains("expect_relation"), poverty200) %>%
  filter(!is.na(poverty200)) %>%
  gather(type, value,  -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, type) %>%
...
scored %>%
  select(contains("expect_relation"), poverty150) %>%
  filter(!is.na(poverty150)) %>%
  gather(type, value,  -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(poverty150, type) %>%
...
```{r 0prelim-may29-21, out.width='100%'}
scored %>%
...
```{r 0prelim-may29-23 }
scored %>%
  select(same_cc, poverty200) %>%
  filter(!is.na(same_cc) & !is.na(poverty200) ) %>%
  group_by(same_cc,poverty200) %>%
  summarize(Count = n()) %>%
  group_by( poverty200) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = same_cc, fill = poverty200))+
  geom_bar(stat = "identity") +
...
```{r 0prelim-may29-24 }
scored %>%
  select(same_cc, poverty150) %>%
  filter(!is.na(same_cc) & !is.na(poverty150) ) %>%
  group_by(same_cc,poverty150) %>%
  summarize(Count = n()) %>%
  group_by( poverty150) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = same_cc, fill = poverty150))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty150~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-28 }
scored %>%
  select(willing_same_cc, poverty200) %>%
  filter(!is.na(willing_same_cc)& !is.na(poverty200)) %>%
  group_by(willing_same_cc,poverty200) %>%
  summarize(Count = n()) %>%
  group_by(poverty200) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = willing_same_cc, fill = poverty200))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty200~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-29 }
scored %>%
  select(willing_same_cc, poverty150) %>%
  filter(!is.na(willing_same_cc)& !is.na(poverty150)) %>%
  group_by(willing_same_cc,poverty150) %>%
  summarize(Count = n()) %>%
  group_by(poverty150) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = willing_same_cc, fill = poverty150))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty150~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-32, out.width='100%'}
scored %>%
  select(contains("barrier_same_cc_"),  poverty200) %>%
  filter(!is.na(poverty200)) %>%
  gather(type, value,  -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, type) %>%
...
```{r 0prelim-may29-33, out.width='100%'}
scored %>%
  select(contains("barrier_same_cc_"),  poverty150) %>%
  filter(!is.na(poverty150)) %>%
  gather(type, value,  -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(poverty150, type) %>%
...
```{r 0prelim-may29-34, out.width='100%'}
scored %>%
...
```{r 0prelim-may29-35, out.width='100%'}
scored %>%
...
```{r 0prelim-may29-37 }
scored %>%
  select(comfort_return, poverty200) %>%
  filter(!is.na(comfort_return)& !is.na(poverty200)) %>%
  group_by(comfort_return, poverty200) %>%
  summarize(Count = n()) %>%
  group_by(poverty200) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  ...
  ggplot(aes(x = Percent, y = comfort_return, fill = poverty200))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty200~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-38 }
scored %>%
  select(comfort_return, poverty150) %>%
  filter(!is.na(comfort_return)& !is.na(poverty150)) %>%
  group_by(comfort_return, poverty150) %>%
  summarize(Count = n()) %>%
  group_by(poverty150) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count),
         comfort_return = factor(comfort_return, 
                                 ...
                                 labels = c(
                                   ...
                                   "A little comfortable and also a little worried or anxious",
                                   "Not at all comfortable and very worried or anxious"))) %>%
  ggplot(aes(x = Percent, y = comfort_return, fill = poverty150))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty150~.)+
  scale_fill_brewer(palette = "Dark2")+
...
scored %>%
  select(contains("know_cc"),  poverty200) %>%
  filter(!is.na(poverty200)) %>%
  gather(type, value, -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, type) %>%
...
```{r 0prelim-may29-43, out.width='100%'}
scored %>%
  select(contains("know_cc"),  poverty150) %>%
  filter(!is.na(poverty150)) %>%
  gather(type, value, -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(poverty150, type) %>%
...
```{r 0prelim-may29-45, out.width='100%'}
scored %>%
...
```{r 0prelim-may29-47 }
scored %>%
  select(lack_cc,  poverty200) %>%
  filter( !is.na(lack_cc) & !is.na(poverty200)) %>%
  group_by(lack_cc, poverty200) %>%
  summarize(Count = n()) %>%
  group_by(poverty200) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = lack_cc, fill = poverty200))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty200~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-48 }
scored %>%
  select(lack_cc,  poverty150) %>%
  filter( !is.na(lack_cc) & !is.na(poverty150)) %>%
  group_by(lack_cc, poverty150) %>%
  summarize(Count = n()) %>%
  group_by(poverty150) %>%
  mutate(Percent = 100*Count/sum(Count)) %>%
  ungroup() %>%
  mutate(Count = ifelse(Count == 0, NA, Count)) %>%
  ggplot(aes(x = Percent, y = lack_cc, fill = poverty150))+
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Percent), x= Percent + 10))+
  facet_grid(poverty150~.)+
  scale_fill_brewer(palette = "Dark2")+
...
```{r 0prelim-may29-58, fig.height=8}
library(ggpubr)
scored = scored %>%
...
```{r 0prelim-may29-59, fig.height=8}
...
```{r 0prelim-may29-60, fig.height=8}
...
```{r 0prelim-may29-61, fig.height=8}
...
```{r 0prelim-may29-62, fig.height=8}
...
```{r 0prelim-may29-63, fig.height=8}
...
```{r 0prelim-may29-64, out.width='100%'}
scored %>%
  select(contains("source_"), all_of(demo_groups), poverty200) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(poverty200)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo, -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(demo, poverty200, type) %>%
  summarize(Count = sum(value),
    Percent = 100*sum(value)/n()) %>%
  ungroup() %>%
  mutate(
...
```{r 0prelim-may29-65, out.width='100%'}
scored %>%
  select(contains("source_"), all_of(demo_groups), poverty150) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1 & !is.na(poverty150)) %>%
  select(-dvalue)%>%
  gather(type, value, -demo, -poverty150) %>%
  filter(!is.na(value)) %>%
  group_by(demo, poverty150, type) %>%
  summarize(Count = sum(value),
    Percent = 100*sum(value)/n()) %>%
  ungroup() %>%
  mutate(
...
```{r 0prelim-may29-66, out.width='100%'}
scored %>%
