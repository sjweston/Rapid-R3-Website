...
```{r}
scored = scored %>%
  mutate(poverty150 = ifelse(is.na(poverty150), poverty150s, poverty150)) %>%
...
black_caregivers %>%
  ...
  select(poverty150, income, children, household, adults, employDecrease) %>%
  group_by(poverty150) %>%
  summarize_all(list(mean = ~mean(.x, na.rm=T),
                     sd = ~sd(.x, na.rm=T),
                     min = ~min(.x, na.rm=T),
                     max = ~max(.x, na.rm=T),
                     n = ~length(which(!is.na(.x))))) %>%
  gather("variable", "value", -poverty150) %>% 
  separate(variable, into = c("variable", "stat")) %>%
  spread(stat, value) %>%
  mutate_at(vars(mean, sd, min, max), papaja::printnum) %>%
  mutate(n = papaja::printnum(n, digits = 0)) %>%
  ...
  kable(.,
        ...
        caption = "Descriptive statistics for demographic variables. Adults calculated by subtracting the number of children from household size; if resulting number was less than 1, the number of adults was imputed as 1.") %>%
  kable_styling(full_width = T) %>%
  group_rows("High Income", 1, 5) %>%
  group_rows("Low Income", 6, 10) 
```
...
black_caregivers %>%
  ...
  gather("variable", "value", -poverty150) %>%
  mutate(value = ifelse(value > 250000, 250000, value)) %>%
  ggplot(aes(x = value, fill = poverty150)) +
...
plot = scored %>%
  ...
  ungroup() %>%
  mutate_at(vars(black), factor, labels = c("non-Black", "Black")) %>%
  mutate_at(vars(poverty150), factor, labels = c("High income", "Low income")) %>%
  group_by(black, poverty150) %>%
  summarize_at(vars(access_telehealth_self, access_telehealth_child, access_mentalhealth), 
               list(mean = ~100*mean(.x, na.rm=T),
                    N = ~length(which(!is.na(.x))))) %>% 
  gather("variable", "value", -black, -poverty150) %>%
  mutate(stat = ifelse(grepl("_N", variable), "N", "percent"),
         variable = str_remove(variable, "_mean"),
         variable = str_remove(variable, "_N")) %>% 
  mutate(variable = factor(variable,
...
plot = scored %>%
  filter(!is.na(black) & !is.na(poverty150)) %>%
  group_by(black, poverty150, Week) %>%
  summarize_at(vars(access_telehealth_self, access_telehealth_child, access_mentalhealth), 
               list(mean = ~100*mean(.x, na.rm=T),
                    N = ~length(which(!is.na(.x))))) %>% 
  ungroup() %>%
  gather("variable", "value", -black, -poverty150, -Week) %>%
  mutate(stat = ifelse(grepl("_N", variable), "N", "percent"),
         variable = str_remove(variable, "_mean"),
         variable = str_remove(variable, "_N")) %>% 
  mutate(variable = factor(variable,
...
plot = scored %>%
  filter(!is.na(black) & !is.na(poverty150)) %>%
  filter(delay_healthcare == 1) %>%
  select(CaregiverID, black, poverty150, starts_with("hc_barrier")) %>%
  group_by(CaregiverID, black, poverty150) %>%
  summarize_all(~max(.x, na.rm=T)) %>%
  ungroup() %>%
  gather(variable, value, -black, -poverty150, -CaregiverID) %>%
  filter(is.finite(value)) %>%
  group_by(black, poverty150, variable) %>%
  summarize(percent = 100*mean(value, na.rm=T),
            N = length(which(!is.na(value)))) %>%
  ungroup() %>%
  mutate_at(vars(black), factor, labels = c("non-Black", "Black")) %>%
