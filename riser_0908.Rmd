...
plot = scored %>%
  ...
  filter(!is.na(Week) & !is.na(poverty200)) %>%
  mutate(Date = case_when(
    Week == 0 ~ as.Date("2020-03-01"),
    TRUE ~ Date)) %>%
  filter(black == "Black") %>%
  select(Week, Date, poverty200, fussy_current_some, fear_current_some) %>%
  gather(variable, value, -Week, -Date, -poverty200) %>%
  filter(!is.na(value)) %>%
  group_by(poverty200, Week, variable) %>%
  summarize(
    Date = min(Date, na.rm = T),
    n = n(),
            percent = round(mean(value)*100,2)) %>%
  ungroup() %>%
  mutate(variable = str_remove(variable, "_current_some"),
         poverty200 = factor(poverty200, 
                             labels = c("Above 2x FPL",
                                       "At or below 2x FPL"))) %>%
  ggplot(aes(x = Date, y = percent, color = poverty200)) +
  geom_line() +
  labs(title = "Caregivers reporting some or a lot of ___",
       caption = "March 1 is used to represent retrospective reports of pre-pandemic mental health")+
  facet_wrap(~variable)+
...
<!--   filter(!is.na(black)) %>% -->
<!--   filter(Week == 0) %>% -->
<!--   select(black, fussy, fear) %>% -->
<!--   group_by(black) %>% -->
<!--   summarise(across( -->
<!--     where(is.numeric), -->
<!--     list(m = mean, s = sd, n = ~length(which(!is.na(.x)))), -->
<!--     na.rm=T)) %>% -->
<!--   gather(variable, value, -black) %>% -->
<!--   separate(variable, into = c("variable", "stat")) %>% -->
<!--   spread(stat, value) %>% -->
<!--   mutate(se = s/sqrt(n), -->
<!--          moe = qt(.975, n-1)*se) %>% -->
<!--   ggplot(aes(x = variable, y = m, group = black, -->
<!--              text = paste0( -->
<!--                "Group:", black, -->
<!--                "\nMean:", papaja::printnum(m), -->
<!--                "\n95% CI low:", papaja::printnum(m-moe), -->
<!--                "\n95% CI high:", papaja::printnum(m+moe) -->
<!--              ))) + -->
<!--   geom_errorbar(aes(ymin = m-moe, ymax = m+moe), -->
<!--                 position = position_dodge(1), -->
<!--                 width = .3) + -->
<!--   geom_point(aes(color = black), position = position_dodge(1)) + -->
<!--   labs(x = NULL, y = "Average rating (out of 100)", color = NULL) + -->
<!--   theme_pubr() -->
...
plot = plot %>%
  ...
  ggplot(aes(x = variable, y = percent, group = poverty200, fill = poverty200,
             text = paste0(
               "Race:", black,
               "\nPoverty status:", poverty200,
               "\nPercent:", papaja::printnum(percent),
               "\nN:", n
             ))) +
  geom_bar(stat = "identity", position = position_dodge(1)) +
  coord_flip() +
...
plot = scored %>%
  ...
  filter(!is.na(black)) %>%
  group_by(black, Week) %>%
  summarize(percent = 100*mean(material_hardship,na.rm=T),
            N = length(which(!is.na(material_hardship)))) %>%
  ungroup() %>%
  filter(!is.nan(percent)) %>%
  mutate_at(vars(black), factor, labels = c("non-Black", "Black")) %>%
  ggplot(aes(x = Week,
