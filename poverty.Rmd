...
prepandemic %>%
  ...
  group_by(emotion) %>%
  nest() %>%
  mutate(test = map(data, function(x) t.test(value~poverty, data = x))) %>%
...
```{r}
chisq.test(week1$poverty, week1$lost_free_lunch)
week1 %>%
  ungroup()%>%
...
```{r}
chisq.test(week1$poverty, week1$access_telehealth)
week1 %>%
  ungroup()%>%
...
```{r}
chisq.test(week1$poverty, week1$insurance)
week1 %>%
  ungroup()%>%
...
```{r}
chisq.test(week1$poverty, week1$child_insurance)
week1 %>%
  ungroup()%>%
