...
covid <- merge (x = master_dem_fl, y = covid, by.x = "CaregiverID", by.y = "CaregiverID", all.y = T)%>%
  select (CaregiverID, StartDate, SurveyType, Week, month, baseline, race_ethnic, poverty_cat, region, contains ("COVID"), pmh_total_crt, cp_total_crt)
...
```{r, echo=FALSE, message = FALSE, warning = FALSE}
covid_know_dg <- covid %>%
  filter (Week >= 29 & SurveyType == 2 & region!= "NA")%>%
  group_by (Week, region)%>%
  summarise (n = n(),
             know_diagnose = sum(COVID.003, na.rm = TRUE)/n, 
             know_hospitalized_died = sum (COVID.004, na.rm = T)/n, 
