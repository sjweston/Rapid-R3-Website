...
weeks = names(tab.inc)
weeks = str_extract(weeks, "Week[0-9]{2}")
weeks = str_remove(weeks, "Week")
weeks = unique(as.numeric(weeks[!is.na(weeks)]))

beginning = "tab.inc %>% kable(., format = \"html\", col.names = c(\"Income (in thounsands per year)\", rep(c(\"Count\", \"Percent\"), length(weeks))), digits = c(0, rep(c(0,1), max_weeks))) %>% kable_styling() %>%"


header = paste0(",\"Week ", weeks, "\" = 2", collapse = "")
