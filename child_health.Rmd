...
scored %>%
  ...
  write.csv(., here("figure data/100920_missWB_race_poverty.csv"))
```
...
plotdata %>%
  select(age, percent) %>%
  write.csv("figure data/100920_missedwb_ages.csv", row.names = F)
```
...
plot = scored %>%
  ...
  mutate(percent = count/n,
         moe = map2_dbl(percent, n, moe.p)) %>%
    mutate_at(vars(percent, moe), ~100*.x) %>%
  ggplot(aes(x = reason, y = percent, text = paste0(round(percent,1), "%"))) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = percent-moe, 
                    ymax = percent+moe),
                width = .5) +
