...
```{r -group-based-23, eval = (params$subset == "single"), results = 'asis'}
spline.data = scored %>%
  mutate(
    SP1 = case_when(
      Week < 15 ~ Week-15,
...
```{r -group-based-39, eval = is.single}
...
```{r -group-based-49, eval = (params$subset == "single"), results = 'asis'}
...
tidy(Mod.Med_group) %>%
  kable(., caption = "mediation model within group")
```
...
```{r}
if(is.single){
  single_header = print("### Correlations
