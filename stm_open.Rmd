...
```{r}
td_gamma27_wide = td_gamma27 %>%
  mutate(topic = paste0("s27_", topic)) %>%
  spread(topic, gamma)
td_gamma30_wide = td_gamma30 %>%
  mutate(topic = paste0("s30_", topic)) %>%
  spread(topic, gamma)
td_gamma34_wide = td_gamma34 %>%
  mutate(topic = paste0("s34_", topic)) %>%
  spread(topic, gamma)

td_challenge_wide = td_gamma27_wide %>%
  full_join(td_gamma30_wide) %>%
  full_join(td_gamma34_wide) 

cor_challenge = td_challenge_wide%>%
  cor(., use = "pairwise") 

cor_challenge = corrplot::corrplot(cor_challenge, order = "hclust")

write.csv(cor_challenge, file = here("STM files/challenge_correlations.csv"))
```
...
```{r}
td_gamma26_wide = td_gamma26 %>%
  mutate(topic = paste0("s26_", topic)) %>%
  spread(topic, gamma)
td_gamma30_wide = td_gamma30 %>%
  mutate(topic = paste0("s30_", topic)) %>%
  spread(topic, gamma)
...
td_helping_wide = td_gamma26_wide %>%
  full_join(td_gamma30_wide) %>%
  full_join(td_gamma34_wide) 

cor_helping = td_helping_wide %>%
  cor(., use = "pairwise") 

cor_helping = corrplot::corrplot(cor_helping, order = "hclust")

write.csv(cor_helping, file = here("STM files/helping_correlations.csv"))
```
