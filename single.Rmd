...
anxiety_plot = trend_days_sheltering_binary(variable = "anxiety_current_some",
                                     ...
                                     groups = c("single"),
                                     group_labels = "Single caregivers")
anxiety_plot$plot
```
...
pov.tabs = xtabs(~ poverty, data = single)
```

Of the `r nrow(single)` caregivers of children with a single, `r pov.tabs[2]` report an income below the poverty threshold for their household size and `r pov.tabs[1]` report an income above the threshold. 
...
anxiety_plot = trend_days_sheltering_cat(variable = "anxiety_current_some",
                                     ...
                                     label = "some or a lot of anxiety",
                                     groups = c("region"),
                                     group_levels = unique(single$region))
anxiety_plot$plot
```
...
single = single %>%
  mutate(nonfamliy_cc = ifelse(hours_nonfamilyCC_current == 0, 
                               "No nonfamily care", 
                               "Nonfamily care")) 
tab_nonfamily_cc = table(single$nonfamliy_cc)
```
...
single = single %>%
  mutate(famliy_cc = ifelse(hours_familyCC_current == 0, 
                               "No family care", 
                               "family care")) 
tab_family_cc = table(single$famliy_cc)
```
...
stats::chisq.test(week1$single, week1$income_decreaed)
week1 %>%
  ungroup()%>%
  select(single, income_decreaed) %>%
  group_by(single) %>%
  summarize(n = n(),
...
```{r}
stats::chisq.test(week1$single, week1$decrease_nonfamilyCC)
week1 %>%
  ungroup()%>%
  select(single, decrease_nonfamilyCC) %>%
  group_by(single) %>%
  summarize(n = n(),
...
```{r}
stats::chisq.test(week1$single, week1$difficulty_basics)
week1 %>%
  ungroup()%>%
  select(single, difficulty_basics) %>%
  group_by(single) %>%
  summarize(n = n(),
...
```{r}
stats::chisq.test(week1$single, week1$delay_healthcare)
week1 %>%
  ungroup()%>%
  select(single, delay_healthcare) %>%
  group_by(single) %>%
  summarize(n = n(),
