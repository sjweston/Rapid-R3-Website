...
meds = c("insurance", "child_insurance", "access_online", "suspected_covid", "diagnosed_covid", "hospital", "hospital_covid", "support_decrease", "lost_free_lunch", "employment_decreased", "losejob_sickleave", "income_decreaed", "difficulty_basics", "minority", "black", "latinx")
...
```{r, results = 'asis'}
model = lmer(anxiety ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(anxiety ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
...
demo.med = scored %>%
  ...
  filter(Week == min(Week)) %>%
    select(CaregiverID, lowincome, all_of(meds))
...
model = lm(Change ~ lowincome, data = change)
broom::tidy(model) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
        digits = c(0,2,2,2,3)) %>%
  kable_styling() 
...
change %>%
  ...
  summarize(m = mean(Change,na.rm=T),
         ...
         cv = qt(p = .975, df = n-1),
         moe = se*cv) %>%
  mutate(lowincome = factor(lowincome, labels = c("Middle to high income", "Low income"))) %>%
  ggplot(aes(x = lowincome, y = m)) +
  geom_bar(aes(fill = lowincome), stat = "identity") +
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lm(Change ~ lowincome + m.value, 
                                       data = d)))
change %>%
  gather("mediator", "m.value", all_of(meds)) %>%
  group_by(mediator) %>%
  ...
  select(mediator, N, fit) %>%
  unnest(cols = c(fit)) %>%
  filter(term %in% c("lowincome", "m.value")) %>%
  mutate(estimate = papaja::printnum(estimate),
         estimate = ifelse(p.value < .05, paste0(estimate,"*"), estimate)) %>%
  select(mediator, term, estimate) %>%
  spread(term, estimate) %>%
...
```{r, results = 'asis'}
model = lmer(depress ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
  mutate(p.value = pt(abs(statistic), df = nrow(model@frame)-2, lower.tail = F)*2) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(depress ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
...
```{r, results = 'asis'}
model = lmer(stress ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
  mutate(p.value = pt(abs(statistic), df = nrow(model@frame)-2, lower.tail = F)*2) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(stress ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
...
```{r, results = 'asis'}
model = lmer(lonely ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
  mutate(p.value = pt(abs(statistic), df = nrow(model@frame)-2, lower.tail = F)*2) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(lonely ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
...
```{r, results = 'asis'}
model = lmer(fussy ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
  mutate(p.value = pt(abs(statistic), df = nrow(model@frame)-2, lower.tail = F)*2) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(fussy ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
...
anxiety_plot = trend_days_sheltering_cat(variable = "anxiety_current_some",
                                     ...
                                     label = "some or a lot of anxiety",
                                     groups = c("region"),
                                     group_levels = unique(lowincome$region))
anxiety_plot$plot
```
...
lowincome = lowincome %>%
  mutate(nonfamliy_cc = ifelse(hours_nonfamilyCC_current == 0, 
                               "No nonfamily care", 
                               "Nonfamily care")) 
tab_nonfamily_cc = table(lowincome$nonfamliy_cc)
```
...
lowincome = lowincome %>%
  mutate(famliy_cc = ifelse(hours_familyCC_current == 0, 
                               "No family care", 
                               "family care")) 
tab_family_cc = table(lowincome$famliy_cc)
```
...
chisq.test(week1$lowincome, week1$income_decreaed)
week1 %>%
  ungroup()%>%
  select(lowincome, income_decreaed) %>%
  group_by(lowincome) %>%
  summarize(n = n(),
...
```{r}
chisq.test(week1$lowincome, week1$decrease_nonfamilyCC)
week1 %>%
  ungroup()%>%
  select(lowincome, decrease_nonfamilyCC) %>%
  group_by(lowincome) %>%
  summarize(n = n(),
...
```{r}
chisq.test(week1$lowincome, week1$difficulty_basics)
week1 %>%
  ungroup()%>%
  select(lowincome, difficulty_basics) %>%
  group_by(lowincome) %>%
  summarize(n = n(),
...
```{r}
chisq.test(week1$lowincome, week1$delay_healthcare)
week1 %>%
  ungroup()%>%
  select(lowincome, delay_healthcare) %>%
  group_by(lowincome) %>%
  summarize(n = n(),
...
```{r, results = 'asis'}
model = lmer(fear ~ Week*lowincome + (1|CaregiverID), data = scored, control = control)
broom::tidy(model) %>%
  mutate(p.value = pt(abs(statistic), df = nrow(model@frame)-2, lower.tail = F)*2) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(., col.names = c("Term", "Coef", "SE", "t", "p"),
...
model =
  tibble(mediator = meds, 
         model = list(function(d) lmer(fear ~ Week*lowincome + Week*m.value + 
                                         (1|CaregiverID), 
                                       data = d, 
                                       control = control)))
scored %>%
