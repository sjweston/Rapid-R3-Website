...
composite_grps_avg = function(variable, groups, data = scored){
  ...
  plot = data %>%
    ...
    mutate(
      time = case_when(
        Week == 0 ~ "Pre-pandemic",
        TRUE ~ "During pandemic"),
      time = factor(time, levels = c(
        "Pre-pandemic",
      "During pandemic"))
      ) %>%
  group_by(CaregiverID, {{groups}}, time) %>%
  ...
  ggplot(aes(x = time,
    ...
    geom_errorbar(aes(ymin = m-moe, ymax = m+moe),
                width = .5,
                position = position_dodge(1)) +
  labs(x = NULL, title = "Average concern for children", y = NULL, fill = NULL) +
  theme_pubr()
...
plot = scored %>%
  ...
  summarize(composite_concern = mean(composite_concern, na.rm=T)) %>%
  group_by(time) %>%
  ...
  guides(fill = F) +
  labs(x = NULL, title = "Average Concern for children", y = NULL) +
  theme_pubr()
...
```{r riser1-11 }
composite_grps_avg(composite_concern, black)
```

#### Race/Ethnicity

```{r riser1-12 }
composite_grps_avg(composite_concern, race_ethnic)
```
...
```{r riser1-13 }
composite_grps_avg(composite_concern, blackIncome)
```
...
### Pre and during pandemic{.tabset}
...
plot = scored %>%
  ...
  group_by(CaregiverID, time) %>%
  summarize(discrimination = mean(discrimination, na.rm=T)) %>%
  group_by(time) %>%
  ...
  geom_errorbar(aes(ymin = m-moe, ymax = m+moe), width = .3) +
  guides(fill = F) +
  labs(x = NULL, title = "Average discrimination", y = NULL) +
  theme_pubr()
...
```{r riser1-59 }
composite_grps_avg(discrimination, black)
```

#### Race/Ethnicity

```{r riser1-60 }
composite_grps_avg(discrimination, race_ethnic)
```

#### Black + Income

```{r riser1-61 }
composite_grps_avg(discrimination, blackIncome)
```
