...
```{r mh-followup-5}
policy_binary(single_cat, material_hardship)
```
...
```{r mh-followup-9}
time_policy_binary(single_cat, material_hardship)
```
...
```{r mh-followup-13}
group_categorical_long(single_cat, category, response, scored_diff)
```
...
```{r mh-followup-17}
time_group_cat_long(single_cat, category, response, 
                    data = filter(scored_diff, category %in% c("house", "utilities", "food")))
```
...
```{r mh-followup-21}
policy_cont(single_cat, num_hardship)
```
...
```{r mh-followup-25}
time_policy_cont(single_cat, num_hardship)
```
