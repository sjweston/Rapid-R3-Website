...
master = master %>%
  mutate(State = map_chr(CC.DEMO.001, identify_state),
         child_age03 = case_when(
           ...
           TRUE ~ NA_character_)) %>%
  select(ProviderID, Week, CC.VACC.003, CC.STIM.002.d, CC.STIM.003.d, CC.OPEN.001, CC.OPEN.002, CC.OPEN.003, CC.OPEN.004, CC.OPEN.005, CC.OPEN.006, CC.OPEN.007, State, FPL.2019.150, RaceGroup, ProviderAge, child_age03) %>%
  ...
  gather("Question", "Response", CC.VACC.003, CC.STIM.002.d, CC.STIM.003.d, CC.OPEN.001, CC.OPEN.002, CC.OPEN.003, CC.OPEN.004, CC.OPEN.005, CC.OPEN.006) %>%
  filter(Response != "") %>%
  filter(Response != "-99")
