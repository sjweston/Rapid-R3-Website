...
sample_nation = scored %>%
  gather(demo, value, all_of(nation_demos)) %>%
  filter(value == 1) %>%
  filter(!is.na(gender)) %>%
  group_by(demo) %>%
  summarize(Count = n()) %>%
...
pop %>%
  ...
  mutate(demo = factor(demo, levels = nation_demos, labels = nation_labels)) %>%
  ggplot(aes(x = demo, y = Percent, group = group)) +
  geom_bar(aes(fill = group),
           stat = "identity", position = "dodge") +
  geom_text(aes(label = round(Percent, 1), y = Percent + 5), position = position_dodge(1)) +
  labs(x = "", fill = "")+
