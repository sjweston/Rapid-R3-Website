...
**For the time your child will be learning remotely, who will be assisting with your child’s online learning?**
...
```{r, results='asis'}
school_managing(employment)$test
```
...
```{r, results = 'asis'}
model.mh = lm(m_health ~ online_only + school_parent + learning_inperson + learningp_parent, 
              data =  mhealth_data)
```
...
broom::tidy(model.mh) %>%
  mutate(
    term = str_replace(term, "(Intercept)","Intercept"),
     ...
     term = str_replace(term, "school_parent","Older child: Parent responsible"),
    term = str_replace(term, "learning_inperson","Younger child: In-person care"),
    term = str_replace(term, "learningp_parent","Younger child: Parent responsible"),
  ) %>%
  mutate(p.value = papaja::printp(p.value)) %>%
  kable(., 
        digits = 2, 
