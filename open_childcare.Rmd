...
```{r -open-childcare-47 }
predict_topics<-estimateEffect(formula = 1:num_topics ~ race_ethnic*mental_health + poverty150*mental_health + s(Week), 
                               stmobj = topics_child, 
                               metadata = open_ended1, 
                               uncertainty = "Global")
```
...
document_classification_1 = cbind(document_classification_1, open_ended1) 
...
scored %>%
  ...
  filter(topic %in% c(9, 6, 26, 18, 12)) %>%
...
```{r -open-childcare-89 }
predict_topics<-estimateEffect(formula = 1:num_topics2 ~ race_ethnic + poverty150 + mental_health + s(Week), 
