...
master = master %>%
  mutate(State = map_chr(DEMO.001, identify_state),
         child_age03 = case_when(
           ...
           TRUE ~ NA_character_)) %>%
  select(CaregiverID, Week, OPEN.001, OPEN.002, OPEN.003, OPEN.004, OPEN.006, OPEN.005, OPEN.007, OPEN.008, OPEN.009, POLICY.030, State, FPL.150, RaceGroup, CaregiverAge, child_age03) %>%
  filter(OPEN.006 == 1) %>%
  arrange(Week) %>%
