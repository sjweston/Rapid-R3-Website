...
master = master %>%
  mutate(State = map_chr(DEMO.001, identify_state),
         child_age03 = case_when(
           ...
           TRUE ~ NA_character_)) %>%
  select(CaregiverID, Week, OPEN.001, OPEN.002, OPEN.003, OPEN.004, OPEN.006, OPEN.005, OPEN.007, OPEN.008, OPEN.009, POLICY.030, State, FPL.2019.UM.150, RaceGroup, CaregiverAge, child_age03) %>%
  filter(OPEN.006 == 1) %>%
  arrange(Week) %>%
  ...
  gather("Question", "Response", OPEN.001, OPEN.002, OPEN.003, OPEN.004, OPEN.005, OPEN.007, OPEN.008, OPEN.009, POLICY.030) %>%
  filter(Response != "")
...
# Q10: When thinking about the people/center that provides your childcare right now, what role, significant contribution or niche do they serve for you and your family?

```{r, results = 'asis'}
master %>%
  filter(grepl("POLICY.030", Question)) %>%
  select(-Question) %>%
  datatable(filter = "top", rownames = F,
            options = list(autoWidth = T))
```
