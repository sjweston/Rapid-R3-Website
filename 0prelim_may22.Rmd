...
which_demo = "blw_poverty"
```

```{r 0prelim-may22-3, eval = (which_demo == "blw_poverty")}
scored = scored %>%
  mutate(
...
```{r 0prelim-may22-4, eval = (which_demo == "race_cat")}
...
```{r 0prelim-may22-6, out.width='100%'}
scored %>%
...
```{r 0prelim-may22-8, out.width='100%'}
scored %>%
...
```{r 0prelim-may22-10, out.width='100%'}
scored %>%
  select(contains("expect_age"), all_of(demo_groups)) %>%
  gather("demo","dvalue", all_of(demo_groups)) %>%
  filter(dvalue == 1) %>%
...
```{r 0prelim-may22-15, out.width='100%'}
scored %>%
...
```{r 0prelim-may22-19, out.width='100%'}
scored %>%
...
```{r 0prelim-may22-23, out.width='100%'}
scored %>%
...
```{r 0prelim-may22-24}
text_data = scored[,c("CaregiverID", demo_groups)] %>%
  left_join(master[master$Week == 7, c("CaregiverID", find_items("OPEN", master))])
```
...
```{r 0prelim-may22-43, fig.height=8}
library(ggpubr)
scored = scored %>% 
...
```{r 0prelim-may22-44, fig.height=8}
library(ggpubr)
scored = scored %>% 
  mutate(lostCC_group = factor(lostCC, 
                               levels = c(0,1), 
