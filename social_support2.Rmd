...
```{r}
scored = scored %>%
  select(CaregiverID, Week, single, poverty150, unemployed, black,
         contains("diff_pay"), 
...
```{r}
plot = scored %>%
  filter(!is.na(mh_food) & !is.na(unemployed)) %>%
  group_by(mh_food, unemployed) %>%
  summarize(count = n()) %>%
  group_by(unemployed)  %>%
  mutate(percent = 100*count/sum(count)) %>%
  mutate(percent = round(percent, 1)) %>%
  ...
  mutate(unemployed = factor(unemployed, 
                             ...
                             labels = c("Employed",
                                        "Unemployed"))) %>%
  ggplot(aes(x = mh_food, y = percent, fill = unemployed)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip()+
  labs(title = "Worried about food", y = "Percent", x = NULL) +
  theme_pubr()
...
plot = scored %>%
    filter(mh_eviction != 6) %>%
filter(!is.na(mh_eviction) & !is.na(unemployed)) %>%
  group_by(mh_eviction, unemployed) %>%
  summarize(count = n()) %>%
  group_by(unemployed)  %>%
  mutate(percent = 100*count/sum(count)) %>%
  mutate(percent = round(percent, 1)) %>%
  ...
  mutate(unemployed = factor(unemployed, 
                             ...
                             labels = c("Employed",
                                        "Unemployed"))) %>%
  ggplot(aes(x = mh_eviction, y = percent, fill = unemployed)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip()+
  labs(title = "Worried about eviction", y = "Percent", x = NULL) +
  theme_pubr()
...
plot = scored %>%
   filter(mh_foreclose != 6) %>%
  filter(!is.na(mh_foreclose) & !is.na(unemployed)) %>%
  group_by(mh_foreclose, unemployed) %>%
  summarize(count = n()) %>%
  group_by(unemployed)  %>%
  mutate(percent = 100*count/sum(count)) %>%
  mutate(percent = round(percent, 1)) %>%
  ...
  mutate(unemployed = factor(unemployed, 
                             ...
                             labels = c("Employed",
                                        "Unemployed"))) %>%
  ggplot(aes(x = mh_foreclose, y = percent, fill = unemployed)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip()+
  labs(title = "Worried about foreclose", y = "Percent", x = NULL) +
  theme_pubr()
...
plot = scored %>%
  ...
  mutate(percent = 100*count/sum(count),
   ...
   mutate(unemployed = factor(unemployed, 
                             ...
                             labels = c("Employed", 
                                        "Unemployed"))) %>%
  ggplot(aes(x = mh_all, y = percent, fill = unemployed)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip()+
  labs(title = "Material hardship", y = "Percent", x = NULL) +
  theme_pubr()
...
plot = scored %>%
  ...
  group_by(Week, unemployed, food) %>%
  summarize(count = n()) %>%
  group_by(Week, unemployed) %>%
  mutate(percent = 100*count/sum(count),
         percent = round(percent,1)) %>%
  filter(food != "No") %>%
  mutate(unemployed = factor(unemployed, levels = c(0,1), labels = c("Employed", "Unemployed"))) %>%
  ggplot(aes(x = Week, y = percent, 
             color = food)) +
  geom_point() +
  geom_line()+
  labs(title = "Percent of caregivers experiencing ___ food insecurity", 
       x = NULL)+
  facet_grid(. ~ unemployed, scales = "free")+
  theme_pubclean()
